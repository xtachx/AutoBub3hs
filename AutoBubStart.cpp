...
int ImageEntropyLauncher(cv::Mat& workingFrame, cv::Mat& comparisonFrame, std::string imageDir, std::string imageFile, std::string comparisonFile, std::vector<std::string>::size_type &seq, bool &isTriggered, bool& imStability, int camera)
{
    ...
    std::string refImg = imageDir + comparisonFile;


    /*Storage and pointers to the ROI in the image*/
    //buf_evalImg = cv::imread(evalImg.c_str());
    ...
    if (seq==1 and singleEntropy>5.0005){
        printf("************ ---> WARNING <--******************\n");
    ...
    if (singleEntropy < 0.02 and seq > evalEntropyThresholdFrames and imStability) {
        isTriggered = true;
...
void SearchForBubbleInFrame( std::vector<std::string> SortedFileList, std::string imageDir, std::string eventSeq, std::vector<cv::RotatedRect>& BubblePixelPos, int& trigFrame, int camera, cv::Mat& workingFrame){
    ...
    cv::Mat cmpFrame;
    //cmpFrame = cv::imread(SortedFileList[0].c_str());
    //std::cout<<"Cmp Frame: "<<SortedFileList[0]<<"\n";
    //debugShow(cmpFrame);
    ...
    if (!notFound) {
        ...
        ****************************************************/
        ...
        /*The names of the evalImage and the refImage*/
        std::string cmpImgPath = std::string(imageDir) + SortedFileList[0];
        std::string srcImgPath = std::string(imageDir) + SortedFileList[i];

        std::string trigNextFramePath = std::string(imageDir) + SortedFileList[i+1];
        cv::Mat trigNextFrame = cv::imread(trigNextFramePath.c_str());
        ...
        LocalizeOMatic(workingFrame, cmpFrame, trigNextFrame, BubblePixelPos, eventSeq.c_str(), imagesIDStoragePath);
        //BubblePixelPos.clear();
